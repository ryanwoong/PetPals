...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribeInventory = onSnapshot(inventoryRef, async (snapshot) => {
      try {
        const inventoryData = [];
  ...
  useEffect(() => {
    const fetchShopItems = async () => {
      try {
        ...
        setShopItems(response.data.items.map(item => ({
          ...item,
          ...
          shopItems.map(item => (
            <Grid.Col span={4} key={item.id}>
              ...
              >
                ...
                >
                  <Image src={coin} h="20px" w="20px" style={{ display: 'inline' }} />
                  {item.price}
                </Text>
                <Button
                  fullWidth
